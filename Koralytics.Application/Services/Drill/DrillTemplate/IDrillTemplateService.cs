...
namespace Koralytics.Application.Services.Drill.DrillTemplate
{
    public interface IDrillTemplateService
    {
        ...
        Task<PagedResult<DrillTemplateDto>> GetTemplatesAsync(int academyId, int currentUserId, TemplateFilterDto filter);
        Task<PagedResult<DrillTemplateDto>> GetTemplatesByCategoryAsync(int categoryId, int academyId, int currentUserId, TemplateFilterDto filter);
        ...
        Task<IEnumerable<DrillCategoryDto>> GetCategoriesAsync();
    }
