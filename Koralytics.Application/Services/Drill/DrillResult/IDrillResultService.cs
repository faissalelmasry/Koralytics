...
namespace Koralytics.Application.Services.Drill.DrillResult
{
    public interface IDrillResultService
    {
        ...
        Task<IEnumerable<DrillResultDto>> GetDrillResultsAsync(int sessionId, int drillId, int currentUserId, string currentUserRole, int currentAcademyId);
        Task<IEnumerable<PlayerAttendanceDto>> GetSessionAttendanceAsync(int sessionId, int currentUserId, string currentUserRole, int currentAcademyId);
